...
f = open('reg_partial_summary.txt', 'r')
tokenarray=[]
line = f.readline()
while len(line) > 0:
    if line[0] != '#':
...
slabelstring = "Fit: $0.64 + 0.016 \\times \\sqrt{\\log (N \\times 6.67 \\times 10^{-5})}$"
...
plt.title("Convergence of Simulated Annealing on (30,60)-Regular Instance")
plt.legend()
plt.grid(True)      # show grid on minor ticks too
plt.tight_layout()
fig.savefig("regular_fits.pdf")